...
plot + geom_label_repel(aes(label=gsub("-.*", "", dds$names)), show.legend = FALSE)

# collapse technical replicates
dds$sample <- factor(gsub("-.*", "", dds$names))
...
plot + geom_label_repel(aes(label=gsub("-.*", "", dds.coll.filtered$names)), show.legend = FALSE)
...
dds <- ddssva
res <- results(dds)
# save(dds, file='dds_Christine-quant-Christine-code.rda')
save(dds, file='dds_Leandros-quant-Christine-code.rda')

diffexp.subset <- as.data.frame(res[which(res$padj <0.1),])# & abs(res$log2FoldChange) > 1),])
...
ensembl90.mmusculus <- useEnsembl(biomart='genes', dataset='mmusculus_gene_ensembl', version=90)
mgi <- getBM(attributes=c('ensembl_gene_id','mgi_symbol','mgi_description'),
             filters='ensembl_gene_id',
             values=ensembl.id,
             mart=ensembl90.mmusculus)
...
# write.csv(diffexp.subset, file='Christine-quant-Christine-code.csv')
write.csv(diffexp.subset, file='Leandros-quant-Christine-code.csv')
